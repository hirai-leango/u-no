...
// 信用重み（S0-2）: エピソードの「信頼できる度合い」を内部計算する。
// ※ 数値はUIに出さない。並び順・（将来）埋め込み選定・index閾値の内部判断にのみ使う。
//
...
export function credibilityScore(review: Review, mutualUids?: Set<string>): number {
  ...
  // 相互称賛は減点（S0-3で検出したペア）
  if (mutualUids?.has(review.fromUserId)) score *= 0.5
  // 具体性（本文の厚み）を微加点：最大 +0.3
  const len = (review.comment ?? '').length
  score += Math.min(len, 300) / 1000
  return score
}
